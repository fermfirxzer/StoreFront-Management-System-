  ...
  return (
    ...
    >
      <Link
        ...
        className={[
          "relative inline-flex h-10 w-10 items-center justify-center rounded-full text-white/80 transition-all duration-150",
      ...
      <div
        className={[
          "absolute right-0 top-full z-50 w-[360px] max-w-[calc(100vw-1.5rem)] origin-top-right pt-3",
          isOpen ? "pointer-events-auto opacity-100 translate-y-0" : "pointer-events-none opacity-0 translate-y-1",
          "transition-all duration-200 ease-apple",
        ].join(" ")}
      >
        <div className="rounded-[24px] border border-white/20 bg-white p-4 text-[#1E1B4B] shadow-[0_24px_60px_rgba(15,23,42,0.2)] backdrop-blur-sm">
            ...
            <>
              <div className="mt-4 max-h-[320px] space-y-3 overflow-y-auto pr-1">
                {cart.items.map((item) => (
                  <div
                    key={item.id}
                    className="flex items-center gap-3 rounded-apple-card bg-[#F8FAFF] p-2"
                  >
                    <Link
                      className="flex min-w-0 flex-1 items-center gap-3 rounded-[16px] p-1 transition hover:bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500"
                      onClick={() => {
                        setIsOpen(false);
                      }}
